        ...
        <section class="section">
            <div class="row">
                <div class="col">
                    <div class="card">
                        <div class="card-body">
                            <h5 class="card-title">Edit Data Tim Kerja</h5>
                            ...
                            <form method="POST" action="{{ route('work-team.edit-submit', $work_team->id) }}">
                                @csrf
                                @method('PUT')
                                <div class="row">
                                        ...
                                        <select name="work_group_id" id="work_group_id"
                                            ...
                                            @foreach ($workGroups as $workGroup)
                                                <option value="{{ $workGroup->id }}"
                                                    {{ old('work_group_id', $work_team->work_group_id) == $workGroup->id ? 'selected' : '' }}>
                                                    {{ $workGroup->name }}
                                                </option>
                                            @endforeach
                                        </select>
                                    ...
                                    <div class="col-12 mb-3">
                                        <label for="name" class="form-label">Nama Tim Kerja <span class="text-danger">*</span></label>
                                        <input type="text" class="form-control @error('name') is-invalid @enderror"
                                            id="name" name="name" value="{{ $work_team->name ?:old('name') }}">
                                        @error('name')
                                            <div class="invalid-feedback">
                                                {{ $message }}
                                            </div>
                                ...
                                <div class="mt-5 mb-2 me-2 text-end">
                                    <button type="button" class="btn btn-secondary" onclick="window.location.href='{{ route('work-team.index') }}'">Kembali</button>
