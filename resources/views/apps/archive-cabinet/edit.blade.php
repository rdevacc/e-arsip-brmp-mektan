        ...
        <section class="section">
            <div class="row">
                <div class="col">
                    <div class="card">
                        <div class="card-body">
                            <h5 class="card-title">Edit Data Lemari Arsip</h5>
                            ...
                            <form method="POST" action="{{ route('archive-cabinet.edit-submit', $archive_cabinet->id) }}">
                                @csrf
                                @method('PUT')
                                <div class="row">
                                        ...
                                        <select name="archive_building_id" id="archive_building_id"
                                            ...
                                            @foreach ($archiveBuildings as $archiveBuilding)
                                                <option value="{{ $archiveBuilding->id }}"
                                                    {{ old('archive_building_id', $archive_cabinet->archive_building_id) == $archiveBuilding->id ? 'selected' : '' }}>
                                    ...
                                    <div class="col-12 mb-3">
                                        <label for="name" class="form-label">Nama Lemari Arsip <span class="text-danger">*</span></label>
                                        <input type="text" class="form-control @error('name') is-invalid @enderror"
                                            id="name" name="name" value="{{ $archive_cabinet->name ?:old('name') }}">
                                        @error('name')
                                            <div class="invalid-feedback">
                                                {{ $message }}
                                            </div>
                                ...
                                <div class="mt-5 mb-2 me-2 text-end">
                                    <button type="button" class="btn btn-secondary" onclick="window.location.href='{{ route('archive-cabinet.index') }}'">Kembali</button>
